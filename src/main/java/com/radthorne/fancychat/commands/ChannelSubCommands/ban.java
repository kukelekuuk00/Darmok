...
public class ban implements SubCommand
{
    ...
    public void handle( CommandSender sender, String label, String[] args )
    {
        if ( args.length < 2 )
        {
            sender.sendMessage( FancyChat.messenger.playerError( "You must provide a player name and channel, like /ch ban g kukelekuuk00" ) );
            return;
        }
        ...
        if ( sender instanceof Player )
        {
            try
            {
                ChannelPermissions.playerCanBan( (Player) sender, channel );
            }
        ...
        player.sendMessage( FancyChat.messenger.playerError( "You have been banned from the " + channel.getName() + " channel." ) );
        sender.sendMessage( FancyChat.messenger.playerHeaderMsg( "You have banned " + player.getName() + " from the " + channel.getName() + " channel." ) );
    }
    ...
    {
        return "fancychat.commands.channel.ban";
    }
