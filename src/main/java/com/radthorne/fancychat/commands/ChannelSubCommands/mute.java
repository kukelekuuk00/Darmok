...
public class mute implements SubCommand
{
    ...
    public void handle( CommandSender sender, String label, String[] args )
    {
        ...
        if ( args.length > 2 )
        {
            sender.sendMessage( FancyChat.messenger.playerError( "You must provide a player name and channel, like /ch mute g kukelekuuk00" ) );
            return;
        }
        ...
        if ( sender instanceof Player )
        {
            try
            {
                ChannelPermissions.playerCanMute( (Player) sender, channel );
            }
        ...
        channel.mute( player );
        sender.sendMessage( FancyChat.messenger.playerHeaderMsg( "Player has been muted in " + channel.getName() + " channel." ) );
    }
    ...
    {
        return "fancychat.commands.channel.mute";
    }
